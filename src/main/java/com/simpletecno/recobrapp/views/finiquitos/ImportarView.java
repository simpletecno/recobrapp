...
public class ImportarView extends VerticalLayout implements View {
    ...
    public ImportarView() {
        Page.getCurrent().setTitle("RECOBRAPP- Importar Finiquitos");
        ...
        handler = (InputStream stream, String fileName, String mimeType, long length) -> {
            ...
            try {
                ...
                stream.read(buffer);
                String filePath = VaadinService.getCurrent().getBaseDirectory().getAbsolutePath() + "/projectfiles/" + ((RecobrAppUI) mainUI).sessionInformation.getStrCompanyId() + "/";
                    ...
                    public void onClose(ConfirmDialog dialog) {
                        if (dialog.isConfirmed()) {
                            ...
                            try {
                                ...
                                stQuery1 = ((RecobrAppUI) UI.getCurrent()).databaseProvider.getCurrentConnection().createStatement();
                                ((RecobrAppUI) mainUI).databaseProvider.getCurrentConnection().setAutoCommit(false);
                                ...
                                ((RecobrAppUI) mainUI).databaseProvider.getCurrentConnection().commit();
                                ((RecobrAppUI) mainUI).databaseProvider.getCurrentConnection().setAutoCommit(true);
                                ...
                                try {
                                    ((RecobrAppUI) mainUI).databaseProvider.getCurrentConnection().rollback();
                                    ((RecobrAppUI) mainUI).databaseProvider.getCurrentConnection().setAutoCommit(true);
